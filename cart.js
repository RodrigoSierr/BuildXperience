...
class Cart {
    ...
    checkAuthState() {
        // Eliminar la redirección a login, permitir carrito para invitados
        // const userData = this.getUserData();
        // if (!userData) {
    ...
    saveCart() {
        ...
        if (userData && userData.email) {
            localStorage.setItem(`cart_${userData.email}`, JSON.stringify(this.items));
        } else {
            localStorage.setItem('cart_guest', JSON.stringify(this.items));
        }
    }
...
// Función universal para obtener el carrito del usuario actual
function getCartGlobal() {
    const userData = getUserData();
    if (!userData || !userData.email) return [];
    return JSON.parse(localStorage.getItem(`cart_${userData.email}`)) || [];
}

// Función universal para guardar el carrito del usuario actual
...
// Función universal para actualizar el contador del carrito en el header
...
// Llamar a la sincronización global del carrito en cada página
...
window.getCartGlobal = getCartGlobal;
window.saveCartGlobal = saveCartGlobal;
